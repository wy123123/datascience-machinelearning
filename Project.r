...
#remove NA columns
dt.train.2=dt.train.1[,colSums(is.na(dt.1))<19622*0.1]
dt.test.2=dt.test.1[,colSums(is.na(dt.1))<20*0.1]
...
confusionMatrix(dt.train.4$classe,predict(mod.4,trainP))
varImp(mod.1)
...
for(i in 1 : 10){
        ...
        #remove NA columns
        dt.train.2=dt.train.1[,colSums(is.na(dt.1))<length(folds.train[[i]])*0.1]
        dt.test.2=dt.test.1[,colSums(is.na(dt.1))<length(folds.test[[i]])*0.1]
        #near near 0 variance columns
        dt.train.3 <- nearZeroVar(dt.train.2,saveMetrics=T)
        dt.train.4 <- dt.train.2[,!dt.train.3$nzv]
        dt.test.3 <- nearZeroVar(dt.test.2,saveMetrics=T)
